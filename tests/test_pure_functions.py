...
def test_clean_json_text():
    ...
    test_cases = [
        ('Here is the JSON response: {"key": "value"}', '{"key": "value"}'),
        (
...
def test_extract_json():
    ...
    test_cases = [
        ...
        # JSON with prefix
        ('Here is JSON: {"key": "value"}', '{"key": "value"}', None),
        # JSON with suffix
        ('{"key": "value"} Hope this helps!', '{"key": "value"}', None),
        # JSON array
        ("[1, 2, 3]", "[1, 2, 3]", None),
        # No JSON
        ("No JSON here", "", "No JSON object or array found in response"),
        # Invalid JSON
        ('{"key": value}', "", "Invalid JSON"),  # Missing quotes
    ]
...
def test_parse_claude_response():
    ...
    assert (
        result is not None
    )  # We don't validate fields here, that's done by EmailAnalysisResponse
