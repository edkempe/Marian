...
def test_semantic_search_threshold_adjustment():
    ...
    def mock_messages_create(*args, **kwargs):
        ...
        if (
            "For short queries, prioritize abbreviations and key terms."
            in system_prompt
        ):
        ...
        return type(
            ...
            {
                "content": [
                    type("Content", (), {"text": '[{"matches": [(0, 0.9, "test")]}]'})
                ]
            },
        )
...
def test_semantic_search_json_parsing():
    ...
    test_cases = [
        ...
        # Empty matches
        ('[{"matches": []}]', lambda matches: len(matches) == 0),
        # Multiple matches
        (
...
def test_semantic_search_response_validation():
    ...
    def mock_invalid_json(*args, **kwargs):
        return type(
            "Response", (), {"content": [type("Content", (), {"text": "invalid json"})]}
        )
