            ...
            for node in ast.walk(tree):
                ...
                if isinstance(node, ast.FunctionDef):
                    ...
                    if not re.match(r"^[a-z][a-z0-9_]*$", name):
                        patterns["snake_case_violations"].append(
                            f"{py_file}:{node.lineno} - {name}"
                        )
                    if len(name) < min_name_length:
                        patterns["too_short_names"].append(
                            f"{py_file}:{node.lineno} - {name}"
                        )
                    if name.lower() in unclear_indicators:
                        patterns["unclear_names"].append(
                            f"{py_file}:{node.lineno} - {name}"
                        )
                ...
                # Check class names
                    ...
                    if not re.match(r"^[A-Z][a-zA-Z0-9]*$", name):
                        patterns["camel_case_violations"].append(
                            f"{py_file}:{node.lineno} - {name}"
                        )
...
def test_code_style():
    ...
    for py_file in project_root.rglob("*.py"):
        ...
        try:
            ...
            assert (
                not trailing_space
            ), f"Trailing whitespace in {py_file}: {trailing_space}"
