...
from database.config import get_analysis_session, get_email_session, init_db
...
def setup_test_dbs(monkeypatch):
    ...
    if "emails" in tables:
        ...
        if missing_columns:
            raise ValueError(
                f"Missing required columns in emails table: {missing_columns}"
            )
...
def test_self_email_analysis(setup_test_dbs):
    ...
    with patch("app_email_self_log.GmailAPI", return_value=mock_gmail), patch(
        "app_email_self_log.EmailAnalyzer", return_value=mock_analyzer
    ):
        ...
        analysis_results = analyzer.analyze_emails(
            emails[:1]
        )  # Only analyze first email
