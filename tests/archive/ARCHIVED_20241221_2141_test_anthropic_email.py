...
def verify_environment():
    ...
    if not env_path.exists():
        raise FileNotFoundError(
            ".env file not found. Please create one with your ANTHROPIC_API_KEY"
        )
    ...
    if not db_path.exists():
        raise FileNotFoundError(
            "email_store.db not found. Please make sure the database exists"
        )
...
class EmailBatchProcessor:
    def __init__(
        ...
        logging.info(
            f"Initialized with batch_size={batch_size}, "
    ...
    def setup_database(self):
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_processing_status ON email_triage(processing_status)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_priority ON email_triage(priority)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_batch_id ON email_triage(batch_id)"
        )
