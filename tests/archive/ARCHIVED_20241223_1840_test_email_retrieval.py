...
def test_fetch_emails_error(mock_gmail):
    ...
    mock_gmail.users.return_value.messages.return_value.list.return_value.execute.side_effect = Exception(
        "API Error"
    )
    messages = fetch_emails(mock_gmail)
    assert messages == []
...
def test_process_email_error(mock_gmail, test_db_session):
    ...
    mock_gmail.users.return_value.messages.return_value.get.side_effect = Exception(
        "API Error"
    )
