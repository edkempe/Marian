...
from tests.test_data.semantic_test_data import get_similar_titles, get_test_items
...
def verify_claude_api():
    ...
    try:
        ...
        if "API_TEST" not in result:
            pytest.skip(
                f"Claude API echo test failed. Expected 'API_TEST' in response, got: {result}"
            )
...
def test_semantic_matches_basic(verify_claude_api):
    ...
    assert any(
        item.title == "How to read a file line by line in Python"
...
def test_semantic_matches_variations(verify_claude_api):
    ...
    assert len(matches) >= 1
    assert any(
        item.title == "Understanding Python decorators" for item, score, _ in matches
    )
...
def test_semantic_matches_context(verify_claude_api):
    ...
    assert len(matches) >= 2
    assert any(
        item.title == "Data Structures for Efficient Memory Usage"
        for item, score, _ in matches
    )
    assert any(
        item.title == "Database Memory Optimization" for item, score, _ in matches
    )
...
def test_semantic_matches_short_form(verify_claude_api):
    ...
    matches = chat.get_semantic_matches(
        "ml", items, threshold=CATALOG_CONFIG["POTENTIAL_MATCH_THRESHOLD"]
    )
    assert len(matches) >= 1
    ...
    matches = chat.get_semantic_matches(
        "machine learning", items, threshold=CATALOG_CONFIG["POTENTIAL_MATCH_THRESHOLD"]
    )
    assert len(matches) >= 1
    assert any(item.title == "ml" for item, score, _ in matches)
    ...
    matches = chat.get_semantic_matches(
        "documentation", items, threshold=CATALOG_CONFIG["POTENTIAL_MATCH_THRESHOLD"]
    )
    assert len(matches) >= 1
