...
def check_doc_versioning(file_path: str) -> Dict[str, Optional[str]]:
    ...
    result = {"version": None, "status": None}
...
def get_all_docs(base_dir: str) -> List[str]:
    ...
    for root, _, files in os.walk(base_dir):
        ...
        for file in files:
            if file.endswith(".md"):
                docs.append(os.path.relpath(os.path.join(root, file), base_dir))
    return docs
...
def test_required_doc_versioning():
    ...
    assert not missing_version, f"Documents missing version: {missing_version}"
    assert not missing_status, f"Documents missing status: {missing_status}"
...
def generate_report_content() -> Tuple[List[Dict], List[str], List[str]]:
    ...
    for doc in docs:
        ...
        if not result["version"] and not result["status"]:
            ...
            has_both.append(
                {"path": doc, "version": result["version"], "status": result["status"]}
            )
...
def generate_markdown_report(
    has_both: List[Dict], missing_version: List[str], missing_status: List[str]
) -> str:
...
def generate_html_report(
    ...
    return template.render(
        title="Documentation Versioning Report", content=html_content
    )
