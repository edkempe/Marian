                ...
                if isinstance(node, ast.FunctionDef):
                    ...
                    for decorator in node.decorator_list:
                        if (isinstance(decorator, ast.Call) and 
                            any(name in ast.unparse(decorator.func)
                            ...
                            if 'method' in ast.unparse(decorator).lower():
                                method = re.search(r'method=[\'"](\w+)[\'"]',
                                                ast.unparse(decorator)).group(1)
...
def test_api_documentation():
    ...
    implemented_paths = {
        f"{endpoint.path}:{endpoint.method.lower()}"
    ...
    for path, methods in spec.get('paths', {}).items():
        for method, details in methods.items():
            ...
            assert details.get('description'), \
                f"Missing description for {method.upper()} {path}"
            
            # Check for examples
            assert details.get('examples') or details.get('requestBody', {}).get('examples'), \
                f"Missing examples for {method.upper()} {path}"
            
            # Verify error responses
            responses = details.get('responses', {})
            assert '400' in responses or '404' in responses or '500' in responses, \
...
def test_logging_standards():
    ...
    for log in log_statements:
        if log['message']:
            ...
            assert any(marker in log['message'] 
                      for marker in ['{', '%s', '%d']), \
                f"Non-structured log message: {log['file']}:{log['line']}"
    
    # Check log configuration
    project_root = Path(__file__).parent.parent
    log_configs = list(project_root.rglob('logging.yaml')) + \
                 list(project_root.rglob('logging.conf'))
    ...
    if log_configs:
        ...
        assert any('rotating' in str(h).lower() 
...
def test_security_compliance():
    ...
    if issues:
        ...
        for issue in issues:
            report.append(
                f"  - {issue['type']} in {issue['file']}:{issue['line']}"
            )
        pytest.fail("\n".join(report))
    ...
    if security_docs:
        with open(security_docs[0]) as f:
            ...
            assert 'api keys' in content, \
                "Security docs missing API key section"
        ...
        try:
            ...
            for node in ast.walk(tree):
                ...
                if (isinstance(node, ast.FunctionDef) and 
                    any('errorhandler' in ast.unparse(d) 
    ...
    if error_docs:
        with open(error_docs[0]) as f:
            ...
            assert 'error codes' in content, \
                "Error docs missing error codes section"
            assert 'handling' in content, \
                "Error docs missing error handling section"
            assert 'logging' in content, \
                "Error docs missing error logging section"
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    ...
                    branches = len([n for n in ast.walk(node) 
                    ...
                    func_lines = len([n for n in ast.walk(node) if isinstance(n, ast.Expr)])
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    if node.name.startswith('test_'):
                        ...
                        # Count assertions
                        stats['assertions'] += len([n for n in ast.walk(node)
                                                  if isinstance(n, ast.Assert)])
                        
                        # Check for parameterization
                        if any('parametrize' in ast.unparse(d)
                              for d in node.decorator_list):
                        ...
                        if any(isinstance(d, ast.Name) and d.id == 'fixture'
                              for d in node.decorator_list):
...
def test_testing_quality():
    ...
    assert any(cfg.exists() for cfg in pytest_configs), \
    ...
    assert any(cfg.exists() for cfg in coverage_configs), \
...
def get_git_info() -> Dict[str, Any]:
    ...
    try:
        ...
        if hooks_dir.exists():
            info['hooks'] = [f.name for f in hooks_dir.iterdir() 
        ...
        result = subprocess.run(['git', 'log', '--format=%s', '-n', '50'],
                              cwd=project_root,
                              capture_output=True,
                              text=True)
        if result.returncode == 0:
        ...
        info['configs'] = [str(cfg.relative_to(project_root))
                          for cfg in git_configs if cfg.exists()]
...
def test_git_workflow():
    ...
    assert '.gitignore' in info.get('configs', []), \
        "Missing .gitignore file"
    
    # Check for CI configuration
    assert any('github/workflows' in cfg for cfg in info.get('configs', [])), \
    ...
    for branch in info.get('branches', []):
        assert any(re.match(pattern, branch) 
...
def get_performance_metrics() -> Dict[str, List[Dict]]:
    ...
    for py_file in project_root.rglob('*.py'):
        ...
        try:
            ...
            for node in ast.walk(tree):
                if (isinstance(node, ast.FunctionDef) and 
                    any('route' in ast.unparse(d) for d in node.decorator_list)):
                    metrics['api_endpoints'].append({
                        'file': str(py_file.relative_to(project_root)),
                        'line': node.lineno,
                        'name': node.name,
                        'has_cache': any('cache' in ast.unparse(d) 
                                       for d in node.decorator_list)
                    })
...
def test_performance_monitoring():
    ...
    for query in metrics['database_queries']:
        ...
        assert not any(term in query['query'].lower() 
                      for term in ['for', 'while']), \
    ...
    assert any((project_root / f).exists() for f in monitoring_files), \
                        ...
                        if isinstance(subnode, ast.Assign):
                            for target in subnode.targets:
                                if isinstance(target, ast.Name):
                                    ...
                                    if any(term in field_name 
                                         for term in sensitive_fields):
...
def test_data_privacy():
    ...
    for field in patterns['personal_data']:
        ...
        related_encryption = any(
            enc['file'] == field['file'] and
            field['field'] in enc['call'].lower()
            for enc in patterns['data_encryption']
        )
        assert related_encryption, \
    ...
    for model_file in model_files:
        ...
        has_validation = any(
            v['file'] == model_file
            for v in patterns['data_validation']
        )
        assert has_validation, \
            f"Missing data validation in {model_file}"
    
    # Check logging practices
    for log in patterns['data_logging']:
        # Verify no sensitive data in logs
        log_str = log['log_call'].lower()
        assert not any(field['field'] in log_str 
                      for field in patterns['personal_data']), \
    ...
    if privacy_docs:
        with open(privacy_docs[0]) as f:
            ...
            assert 'user rights' in content, \
                "Privacy docs missing user rights section"
    ...
    for path in project_root.rglob('*'):
        ...
        if path.is_file() and path.suffix == '.py':
            patterns['files'].append({
                'name': path.name,
                'path': str(rel_path),
                'matches_convention': bool(re.match(conventions['files'], path.name))
            })
            ...
            try:
                ...
                patterns['modules'].append({
                    'name': module_name,
                    'path': str(rel_path),
                    'matches_convention': bool(re.match(conventions['modules'], module_name))
                })
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.ClassDef):
                        patterns['classes'].append({
                            ...
                            'matches_convention': bool(re.match(conventions['classes'], node.name))
                        })
                    elif isinstance(node, ast.FunctionDef):
                        patterns['functions'].append({
                            'name': node.name,
                            'path': f"{rel_path}:{node.lineno}",
                            'matches_convention': bool(re.match(conventions['functions'], node.name))
                        })
            except Exception as e:
            ...
            patterns['folders'].append({
                'name': path.name,
                'path': str(rel_path),
                'matches_convention': bool(re.match(conventions['folders'], path.name))
            })
...
def test_naming_standards():
    ...
    missing_docs = [
        section for name, section in naming_sections.items()
        if section not in standards_content
    ]
    
    if missing_docs:
        pytest.fail(f"Standards documentation missing sections: {', '.join(missing_docs)}")
...
def test_industry_standards_compliance():
    ...
    for pattern in standards['code_organization']['module_patterns']:
        if any(d.name == pattern.rstrip('/') for d in project_root.iterdir() if d.is_dir()):
    ...
    if readme.exists():
        with open(readme) as f:
            content = f.read().lower()
            for section, desc in standards['documentation']['required_sections'].items():
                if section.lower() in content:
                    implemented_standards['documentation'].append(f"Section: {section}")
                else:
                    missing_standards['documentation'].append(f"Section: {section} ({desc})")
    
    # Check development workflow
    github_dir = project_root / '.github'
    if github_dir.exists():
        for item, desc in standards['development_workflow']['review_process'].items():
            if any(item.lower() in f.name.lower() 
                  for f in github_dir.rglob('*') if f.is_file()):
                implemented_standards['development_workflow'].append(f"Process: {item}")
            else:
                missing_standards['development_workflow'].append(f"Process: {item} ({desc})")
        ...
        with open(security_file) as f:
            ...
            for practice, desc in standards['security']['required_practices'].items():
                if practice.lower() in content:
                    ...
                    missing_standards['security'].append(f"Practice: {practice} ({desc})")
    ...
    if any((project_root / f).exists() for f in monitoring_files):
        ...
        missing_standards['performance'].append("Monitoring: No monitoring configuration")
