...
def assert_schema_equal(migration_inspector, model_inspector):
    ...
    for table in migration_tables:
        ...
        # Check columns match
        assert (
            migration_details["columns"].keys() == model_details["columns"].keys()
        ...
        for midx in migration_details["indexes"]:
            ...
            assert any(
                idx["name"] == midx["name"]
                and idx["unique"] == midx["unique"]
                and sorted(idx["column_names"]) == midx["column_names"]
        ...
        assert set(mpk["constrained_columns"]) == set(
        ...
        mfks = sorted(
            ...
            key=lambda x: (
                x["referred_table"],
                tuple(sorted(x["constrained_columns"])),
            ),
        )
        fks = sorted(
            fks,
...
def find_code_references(table_name: str, column_name: str = None) -> bool:
    ...
    for pattern in patterns:
        result = os.system(
            f'grep -r "{pattern}" {project_root}/models {project_root}/services {project_root}/src > /dev/null 2>&1'
        )
...
def check_unused_schema_elements(
    inspector: Any,
...
def print_schema_usage_report(migration_inspector: Any):
    ...
    report_data = {"unused_tables": unused_tables, "unused_columns": unused_columns}
        ...
        with context.begin_transaction():
            ...
            op.create_table(
                ...
                Column(
                    ...
                    String(500),
                    nullable=True,
                    server_default=EMAIL_DEFAULTS["EMAIL_SUBJECT"],
                ),
            ...
            op.create_index(
                "idx_relationships_source", "item_relationships", ["source_id"]
            )
            op.create_index(
                "idx_relationships_target", "item_relationships", ["target_id"]
            )
            op.create_index(
                "idx_relationships_type", "item_relationships", ["relationship_type"]
            )
            ...
            op.create_table(
                ...
                ForeignKeyConstraint(
                    ["source_id"], ["asset_catalog_items.id"], ondelete="CASCADE"
                ),
                ForeignKeyConstraint(
                    ["target_id"], ["asset_catalog_items.id"], ondelete="CASCADE"
                ),
