...
class ComplexityVisitor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node):
        ...
        if self.class_stack:
            ...
            lines = len(ast.unparse(node).split("\n"))
            self.metrics.max_function_lines = max(
                self.metrics.max_function_lines, lines
            )
...
THRESHOLD_MULTIPLIERS = {
    ...
    ".md": {
        ...
        "size": {"warning": 1.5, "error": 1.2},  # Warning at 112.5KB  # Error at 180KB
    },
    ".rst": {
        "lines": {"warning": 1.5, "error": 2.0},
        "size": {"warning": 1.5, "error": 2.0},
    },
    # Default multiplier for other files
    "default": {"lines": 1.0, "size": 1.0},
}


def get_thresholds(file_suffix: str) -> Dict[str, Dict[str, int]]:
    """Get warning and error thresholds for a file type."""
    multiplier = THRESHOLD_MULTIPLIERS.get(
        file_suffix, THRESHOLD_MULTIPLIERS["default"]
    )
...
def print_file_statistics(stats: List[Dict]) -> None:
    ...
    for stat in python_files[:10]:
        ...
        print(
            f"  Classes: {metrics.class_count} (longest: {metrics.max_class_lines} lines)"
        )
        print(
            f"  Methods: {metrics.method_count} (longest: {metrics.max_method_lines} lines)"
        )
        print(
            f"  Functions: {metrics.function_count} (longest: {metrics.max_function_lines} lines)"
        )
...
def test_file_size_warnings(capsys):
    ...
    for stat in stats:
        ...
        if stat["lines"] > thresholds["warning"]["lines"]:
            warnings.append(
                f"WARNING: {stat['path']} has {stat['lines']} lines (threshold: {thresholds['warning']['lines']})"
            )
        if stat["size"] > thresholds["warning"]["size"]:
            warnings.append(
                f"WARNING: {stat['path']} is {stat['size']/1024:.1f}KB (threshold: {thresholds['warning']['size']/1024:.1f}KB)"
            )
...
def test_file_size_errors():
    ...
    for stat in stats:
        ...
        if stat["lines"] > thresholds["error"]["lines"]:
            errors.append(
                f"ERROR: {stat['path']} has {stat['lines']} lines (threshold: {thresholds['error']['lines']})"
            )
        if stat["size"] > thresholds["error"]["size"]:
            errors.append(
                f"ERROR: {stat['path']} is {stat['size']/1024:.1f}KB (threshold: {thresholds['error']['size']/1024:.1f}KB)"
            )
    ...
    for stat in stats:
        if not stat["is_doc"] and stat["path"].endswith(".py"):
            ...
            for metric, value in metrics.__dict__.items():
                if (
                    metric in COMPLEXITY_THRESHOLDS
                    and value > COMPLEXITY_THRESHOLDS[metric]
                ):
                    warnings.append(
                        f"WARNING: {stat['path']} has high {metric} ({value})"
                    )
