...
def test_api_documentation():
    ...
    for path, methods in spec.get("paths", {}).items():
        for method, details in methods.items():
            assert details.get(
                "description"
            ), f"Missing description for {method} {path}"

            if details.get("parameters"):
                for param in details["parameters"]:
                    assert param.get(
                        "description"
                    ), f"Missing parameter description in {method} {path}"

            assert details.get(
                "responses"
            ), f"Missing response schemas for {method} {path}"
            ...
            for status, response in success_responses.items():
                assert response.get("examples") or response.get(
                    "example"
