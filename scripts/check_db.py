...
def check_database():
    ...
    with Session(engine) as session:
        ...
        # Get personal emails
        print("\nPersonal Emails:")
        personal_emails = session.query(Email).filter(Email.labels.like('%CATEGORY_PERSONAL%')).all()
        ...
        label_167_emails = session.query(Email).filter(Email.labels.like('%Label_167%')).all()
        ...
        # Get some statistics
        print("\nEmail Statistics:")
        emails_with_attachments = session.query(func.count(Email.id)).filter(Email.has_attachments == True).scalar()
        ...
        for label, count in sorted(label_count.items(), key=lambda x: x[1], reverse=True)[:10]:
            print(f"{label}: {count}")
