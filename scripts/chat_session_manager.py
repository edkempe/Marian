        ...
        try:
            ...
            if match:
                ...
                if version >= min_version:
                    return cmd
                print(
                    f"Warning: {cmd} version {'.'.join(map(str, version))} "
...
def run_pip_list() -> str:
    ...
    try:
        result = subprocess.run(
            f"{pip_cmd} list".split(), capture_output=True, text=True, check=True
        )
        return result.stdout.strip()
    except subprocess.CalledProcessError as e:
...
def get_recent_changes(
    num_commits: int = SESSION_CONFIG["GIT_COMMITS_TO_CHECK"],
) -> Dict[str, List[str]]:
    """Get recent git changes."""
    changes = {"commits": [], "files": [], "stats": ""}
...
def get_test_status() -> Dict[str, str]:
    ...
    status = {"passing": "unknown", "coverage": "unknown", "failures": []}

    try:
        python_cmd = get_python_command()
        result = run_command(
            [python_cmd, "-m", "pytest", SESSION_CONFIG["TEST_PATH"], "--quiet"]
        )
...
def get_environment_changes() -> Dict[str, List[str]]:
    ...
    changes = {"new_deps": [], "config": [], "database": []}
    ...
    if reqs_file.exists():
        reqs = run_command(
            [
                "git",
                "diff",
                f'HEAD~{SESSION_CONFIG["GIT_COMMITS_TO_CHECK"]}',
                "--",
                str(reqs_file),
            ]
        )
...
def run_session_start_checks():
    ...
    if not session_log.exists():
        session_log.write_text(
            f"""# Development Session Log - {today}
...
def update_next_session(
    ...
    if Path("BACKLOG.md").exists():
        ...
        if tasks:
            content.extend(
                ["", "## Next Steps", "High priority tasks from BACKLOG.md:"]
            )
