...
def extract_json(text: str) -> Tuple[str, Optional[str]]:
    ...
    try:
        ...
        return (
            "",
            f"No closing {close_char} found for JSON {'array' if is_array else 'object'}",
        )
...
def parse_claude_response(
    response_text: str, error_context: Optional[Dict[str, Any]] = None
    ...
    try:
        ...
        if error_context:
            logger.error(
                ...
                extra={**(error_context or {}), "raw_response": response_text},
            )
        return None
