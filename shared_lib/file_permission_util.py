...
class FilePermissionManager:
    ...
    def __init__(
        ...
        self.enforce_permissions = enforce_permissions if enforce_permissions is not None else ENFORCE_PERMISSIONS
        self.file_permissions = file_permissions if file_permissions is not None else DEFAULT_FILE_PERMISSIONS
        self.dir_permissions = dir_permissions if dir_permissions is not None else DEFAULT_DIR_PERMISSIONS
        self.session_id = session_id
        
        # Validate permission values
        if not 0 <= self.file_permissions <= MAX_PERMISSION_MODE:
            raise ValueError(f"File permissions must be between 0 and {MAX_PERMISSION_MODE:o}")
        if not 0 <= self.dir_permissions <= MAX_PERMISSION_MODE:
            raise ValueError(f"Directory permissions must be between 0 and {MAX_PERMISSION_MODE:o}")
        ...
        if self.dir_permissions & REQUIRED_DIR_MODES != REQUIRED_DIR_MODES:
            raise ValueError("Directory permissions must include read, write, and execute access")
            
        # Log initialization
        log_security_event(logger, "permission_manager_initialized", {
    ...
    def check_directory_permissions(self, path: Path) -> None:
        ...
        try:
            ...
            if not os.access(path, REQUIRED_DIR_MODES):
                log_security_event(logger, "insufficient_directory_permissions", {
                    "path": str(path),
            ...
            if self.enforce_permissions:
                ...
                if current_mode != self.dir_permissions:
                    ...
                    log_security_event(logger, "directory_permissions_fixed", {
                        "path": str(path),
            ...
            log_security_event(logger, "directory_permission_error", {
                "path": str(path),
                "error": str(e),
                "session_id": self.session_id
            })
    ...
    def check_file_permissions(self, path: Path) -> None:
        ...
        try:
            ...
            if not os.access(path, REQUIRED_FILE_MODES):
                log_security_event(logger, "insufficient_file_permissions", {
                    "path": str(path),
            ...
            if self.enforce_permissions:
                ...
                if current_mode != self.file_permissions:
                    ...
                    log_security_event(logger, "file_permissions_fixed", {
                        "path": str(path),
                        "old_mode": f"{current_mode:o}",
            ...
            log_security_event(logger, "file_permission_error", {
                "path": str(path),
                "error": str(e),
                "session_id": self.session_id
            })
    ...
    def create_directory(self, path: Path) -> None:
        ...
        try:
            ...
            if self.enforce_permissions:
                os.chmod(path, self.dir_permissions)
                log_security_event(logger, "directory_created", {
                    "path": str(path),
            ...
            log_security_event(logger, "directory_creation_failed", {
                "path": str(path),
                "error": str(e),
                "session_id": self.session_id
            })
    ...
    def create_file(self, path: Path) -> None:
        ...
        try:
            ...
            if self.enforce_permissions:
                os.chmod(path, self.file_permissions)
                log_security_event(logger, "file_created", {
                    "path": str(path),
            ...
            log_security_event(logger, "file_creation_failed", {
                "path": str(path),
                "error": str(e),
                "session_id": self.session_id
            })
    ...
    def copy_file(self, src: Path, dst: Path) -> None:
        ...
        try:
            ...
            if self.enforce_permissions:
                ...
                log_security_event(logger, "file_copied", {
                    "source": str(src),
                    "destination": str(dst),
                    "permissions": f"{self.file_permissions:o}",
                    "session_id": self.session_id
            ...
            log_security_event(logger, "file_copy_failed", {
                "source": str(src),
                "destination": str(dst),
                "error": str(e),
                "session_id": self.session_id
    ...
    def truncate_file(self, path: Path) -> None:
        ...
        try:
            ...
            if self.enforce_permissions:
                os.chmod(path, self.file_permissions)
                log_security_event(logger, "file_truncated", {
                    "path": str(path),
                    "permissions": f"{self.file_permissions:o}",
                    "session_id": self.session_id
                })
        except OSError as e:
            log_security_event(logger, "file_truncate_failed", {
                "path": str(path),
                "error": str(e),
                "session_id": self.session_id
            })
