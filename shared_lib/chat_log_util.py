...
from shared_lib.logging_util import setup_logging, log_security_event
...
from shared_lib.file_constants import (
    ...
    DEFAULT_DIR_PERMISSIONS as DIR_PERMISSIONS_DEFAULT,
    FILE_ENCODING,
    TEMP_SUFFIX,
...
LOG_PERMISSIONS = int(os.getenv("CHAT_LOG_PERMISSIONS", f"{FILE_PERMISSIONS_DEFAULT:o}"), 8)
DIR_PERMISSIONS = int(os.getenv("CHAT_DIR_PERMISSIONS", f"{DIR_PERMISSIONS_DEFAULT:o}"), 8)
LOG_ROTATION_SIZE_MB = int(os.getenv("CHAT_LOG_ROTATION_SIZE_MB", str(MAX_FILE_SIZE_MB)))
ENFORCE_PERMISSIONS = os.getenv("CHAT_ENFORCE_PERMISSIONS", "1") == "1"  # Enable by default
...
class ChatLogger:
    ...
    def log_interaction(
        ...
        log_entry = {
            ...
            "role": role,
            "user_input": user_input[:MAX_INPUT_LENGTH],  # Truncate if somehow bypassed validation
    ...
    def rotate_logs(self, max_size_mb: Optional[int] = None) -> bool:
        ...
        if not isinstance(max_size, int) or max_size <= 0:
            ...
            log_security_event(logger, "invalid_log_rotation_size", {
        ...
        try:
            ...
            try:
                ...
                log_security_event(logger, "log_rotation_started", {
                ...
                log_security_event(logger, "log_rotation_copy_failed", {
                    "error": str(e),
                    "original_file": str(self.log_path),
                    "rotated_file": str(rotated_path),
                    "session_id": self.session_id
            ...
            try:
                ...
                log_security_event(logger, "log_rotation_completed", {
                    "original_file": str(self.log_path),
                    "rotated_file": str(rotated_path),
                    "session_id": self.session_id
                })
                ...
                log_security_event(logger, "log_rotation_truncate_failed", {
                    "error": str(e),
                    "original_file": str(self.log_path),
                    "rotated_file": str(rotated_path),
                    "session_id": self.session_id
            ...
            log_security_event(logger, "log_rotation_failed", {
                "error": str(e),
                "original_file": str(self.log_path),
                "session_id": self.session_id
            })
