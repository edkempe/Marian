...
def verify_schema(engine: Engine, model_class: Type[DeclarativeMeta]) -> None:
    ...
    if missing_in_db:
        raise ValueError(f"Fields in model but missing in database {table_name}: {missing_in_db}")
    if missing_in_model:
        raise ValueError(f"Fields in database but missing in model {table_name}: {missing_in_model}")
