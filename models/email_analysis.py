...
class PriorityModel(BaseModel):
    ...
    score: int = Field(
        ...,
        ge=ANALYSIS_VALIDATION["PRIORITY_SCORE"]["MIN"],
        le=ANALYSIS_VALIDATION["PRIORITY_SCORE"]["MAX"],
        description="Priority score from 1-5",
    )
    reason: str = Field(
        ...,
        min_length=ANALYSIS_VALIDATION["TEXT_LENGTH"]["MIN"],
...
class ActionModel(BaseModel):
    ...
    deadline: Optional[str] = Field(
        None, pattern=ANALYSIS_DATE_PATTERNS["ISO_DATE_OR_EMPTY"]
    )
...
class EmailAnalysisResponse(BaseModel):
    ...
    action_deadline: Optional[str] = Field(
        None, pattern=ANALYSIS_DATE_PATTERNS["ISO_DATE_OR_EMPTY_OR_ASAP"]
    )
    ...
    sentiment: str = Field(
        ..., pattern=f"^({'|'.join(ANALYSIS_SENTIMENT_TYPES.values())})$"
    )
    confidence_score: float = Field(
        default=ANALYSIS_DEFAULTS["CONFIDENCE_SCORE"],
...
class EmailAnalysis(Base):
    ...
    def from_api_response(
        ...
        return cls(
            ...
            analysis_date=now,
            analyzed_date=now,
            prompt_version="1.0",  # TODO: Get from config
            ...
            ref_docs=json.dumps([]),  # TODO: Implement ref docs
