...
MENU_OPTIONS = {
    1: {
        ...
        "help": "Search catalog items by title or content",
    },
...
COMMANDS = {
    ...
    "clear": {"format": "clear", "help": "Clear the screen", "example": "clear"},
    "exit": {"format": "exit", "help": "Exit the program", "example": "exit"},
    "quit": {"format": "quit", "help": "Exit the program", "example": "quit"},
    "q": {"format": "q", "help": "Exit the program", "example": "q"},
    "create_tag": {
...
class CatalogInteractive:
    ...
    def create_command_completer(self):
        ...
        active_items = (
            session.query(CatalogItem.title).filter(CatalogItem.deleted == False).all()
        )
        ...
        while True:
            try:
                ...
                if not choice.isdigit() or int(choice) not in MENU_OPTIONS:
                    print(
                        "Please enter a number between 1 and 9 or 'q'/'quit' to quit (ESC to exit)"
                    )
                    continue
    ...
    def handle_menu_option(self, option: int) -> bool:
        ...
        if option == 1:  # Catalog item search
            ...
            include_archived = (
                self.session.prompt("\nInclude archived tags? (y/n) [n]: ")
                .strip()
                .lower()
                == "y"
            )
            response = self.chat.process_input(
                "list_tags", "archived" if include_archived else ""
            )
            print(response)
            return False
