...
def inspect_prompts_db():
    ...
    try:
        ...
        for prompt in prompts:
            ...
            print(
                f"Average Confidence: {prompt.average_confidence_score:.2f}"
                if prompt.average_confidence_score
                else "Average Confidence: N/A"
            )
            print(
                f"Average Response Time: {prompt.average_response_time_ms:.0f}ms"
                if prompt.average_response_time_ms
                else "Average Response Time: N/A"
            )
            print(
                f"Failure Rate: {prompt.failure_rate*100:.1f}%"
                if prompt.failure_rate is not None
