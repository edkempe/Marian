...
from shared_lib.constants import API_CONFIG, CATALOG_CONFIG
...
class CatalogChat:
    ...
    def __init__(
        ...
        self.enable_semantic = (
            ...
            else CATALOG_CONFIG["ENABLE_SEMANTIC"]
        )
        ...
        self.test_logger.info(
            f"System State: mode={mode}, db_path={db_path}, chat_log={chat_log}"
        )
    ...
    def check_semantic_duplicates(
        self, session, title: str, existing_items: list
    ) -> tuple:
        ...
        try:
            ...
            matches = self.get_semantic_matches(
                title,
                existing_items,
    ...
    def get_semantic_matches(
        self, text: str, items: list, threshold: float = None
    ) -> list:
...
- "Python Beginner's Class" should score 0.8-0.95 as it's more basic"""
                    ...
                    for match in result["matches"]:
                        ...
                        if 0 <= idx < len(items) and score >= threshold:
                            ...
                            self.test_logger.debug(
                                f"Added match: item[{idx}] (score: {score})"
                            )
    ...
    def add_item(
        ...
        try:
            ...
            existing_items = (
                session.query(CatalogItem).filter(CatalogItem.deleted == False).all()
            )

            if not force:
                has_dups, duplicates, potential_matches = (
                    self.check_semantic_duplicates(session, title, existing_items)
                )
                ...
                if potential_matches:
                    ...
                    raise ValueError(
                        CATALOG_CONFIG["ERROR_MESSAGES"][
                            "POTENTIAL_MATCH_WARNING"
                        ].format(matches=match_info)
                    )

            # Create new item
            item = CatalogItem(
                title=title, content=content, description=description, status="draft"
            )
            session.add(item)
            ...
            if tags:
                for tag_name in tags:
                    ...
                    tag = (
                        session.query(Tag)
                        .filter(Tag.name.ilike(tag_name), Tag.deleted == False)
                        .first()
                    )
    ...
    def archive_item(self, session, title: str) -> None:
        ...
        item = (
            ...
            .filter(CatalogItem.title == title, CatalogItem.deleted == False)
            .first()
        )

        if not item:
            raise ValueError(
                CATALOG_CONFIG["ERROR_MESSAGES"]["ARCHIVE_ERROR"].format(
    ...
    def process_input(self, command: str, args: str) -> str:
        ...
        try:
            if command == "archive":
                ...
                return "\n".join(
                    f"{item.title}: {item.content[:100]}..." for item in items
                )
                ...
                items = (
                    ...
                    .order_by(CatalogItem.modified_date.desc())
                    .all()
                )
                if not items:
                ...
                return "\n".join(
                    f"{tag.name} {'(archived)' if tag.deleted else ''}" for tag in tags
                )
                ...
                # Check for semantic duplicates
                has_duplicates, duplicates, potential_matches = (
                    self.check_semantic_duplicates(session, title, all_items)
                )
                if has_duplicates:
                    if self.interactive:
                        ...
                        similar_list = "\n".join(
                            f"- {item[0].title} ({'archived' if item[0].deleted else 'active'})"
                ...
                existing_item = (
                    session.query(CatalogItem)
                    .filter(CatalogItem.title.ilike(title))
                    .first()
                )
                if existing_item:
                ...
                archived_item = (
                    session.query(CatalogItem)
                    .filter(CatalogItem.title.ilike(title), CatalogItem.deleted == True)
                    .first()
                )
                if archived_item:
                    if self.interactive:
                        print(
                            f"\nFound archived item with title '{archived_item.title}'. Would you like to restore it? (y/n): ",
                            end="",
                        )
                ...
                # Check for semantic duplicates
                has_duplicates, duplicates, potential_matches = (
                    self.check_semantic_duplicates(session, tag_name, all_tags)
                )
                if has_duplicates:
                    if self.interactive:
                        ...
                        similar_list = "\n".join(
                            f"- {tag[0].name} ({'archived' if tag[0].deleted else 'active'})"
                ...
                existing_catalog_tag = (
                    session.query(CatalogTag)
                    .filter(
                        CatalogTag.catalog_id == item.id, CatalogTag.tag_id == tag.id
                    )
                    .first()
                )
                ...
                archived_item = (
                    session.query(table)
                    .filter(
                        getattr(table, name_field).ilike(name), table.deleted == True
                    )
                    .first()
                )
                ...
                session.commit()
                return (
                    f"Restored {'tag' if command == 'restore_tag' else 'item'}: {name}"
                )
    ...
    def semantic_search(self, query: str) -> List[CatalogItem]:
        ...
        try:
            ...
            if analysis["search_terms"]:
                ...
                base_query = base_query.filter(
                    CatalogItem.deleted == False, CatalogItem.status != "archived"
                )
                ...
                if "tags" in analysis["filters"]:
                    for tag in analysis["filters"]["tags"]:
                        base_query = (
                            base_query.join(CatalogTag)
                            .join(Tag)
                            .filter(Tag.name.ilike(f"%{tag}%"), Tag.deleted == False)
                        )
    ...
    def process_input(self, user_input: str) -> dict:
        ...
        try:
            ...
            error_context = {"error_type": "catalog_chat", "user_input": user_input}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-tests", action="store_true", help="Skip running integration tests"
    )
    parser.add_argument(
        "--no-semantic",
        action="store_true",
        help="Disable semantic checking for duplicates and search",
    )
    ...
    if not args.no_tests:
        ...
        test_logger.info(
            f"System State: mode=cli, db_path={chat.db_path}, chat_log={chat.chat_log}, semantic_enabled={chat.enable_semantic}"
        )
